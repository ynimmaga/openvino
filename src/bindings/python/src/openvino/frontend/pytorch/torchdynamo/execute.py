...
def execute(
    ...
    *args,
    ...
    if executor == "openvino":
        return openvino_execute_partitioned(gm, *args, options=options, executor_parameters=executor_parameters)
...
def openvino_execute(gm: GraphModule, *args, executor_parameters=None, partition_id, options):
    ...
    if use_cache and (partition_id in compiled_cache):
        ...
        compiled = openvino_compile(gm, *args, options, model_hash_str=model_hash_str)
...
class OpenVINOGraphModule(torch.nn.Module):
    def __init__(self, gm, partition_id, use_python_fusion_cache, options, model_hash_str: str = None):
        super().__init__()
        self.gm = gm
...
def partition_graph(gm: GraphModule, options, use_python_fusion_cache: bool, model_hash_str: str = None):
    ...
    for node in gm.graph.nodes:
        ...
        if node.op == "call_module" and "fused_" in node.name:
            ...
            gm.add_submodule(
                node.target,
                OpenVINOGraphModule(openvino_submodule, partition_id, use_python_fusion_cache, options,
                                    model_hash_str=model_hash_str),
            )
...
def openvino_execute_partitioned(gm: GraphModule, *args, options, executor_parameters=None):
    executor_parameters = executor_parameters or DEFAULT_OPENVINO_PYTHON_CONFIG
    ...
    if signature not in partitioned_modules:
        partitioned_modules[signature] = partition_graph(gm, options, use_python_fusion_cache=use_python_fusion_cache,
                                                         model_hash_str=model_hash_str)
