...
OutputVector translate_stack(const NodeContext& context) {
    ...
    auto dim = context.mark_node(ov::op::v0::Constant::create(element::i32, Shape{}, {0}));

    auto stack_input0 = context.mark_node(std::make_shared<ov::op::v0::Unsqueeze>(context.get_input(0), dim));
    auto stack_input1 = context.mark_node(std::make_shared<ov::op::v0::Unsqueeze>(context.get_input(1), dim));
